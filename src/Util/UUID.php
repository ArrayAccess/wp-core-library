...
class UUID
{
    ...
    public static function calculateNamespaceAndName(
        ...
        if ($version === null) {
            throw new InvalidArgumentException(
                __('Invalid namespace', 'arrayaccess')
            );
        }
    ...
    public static function generate(
        ...
        if ($type === self::UUID_TYPE_MD5 || $type === self::UUID_TYPE_SHA1) {
            if (!$hash) {
                $hash = match ($type) {
                    self::UUID_TYPE_MD5 => md5(Consolidator::randomBytes(16)),
                    default => sha1(Consolidator::randomBytes(16)),
                };
            } elseif ($type === self::UUID_TYPE_SHA1) {
                if (strlen($hash) < 40 && $version !== 5) {
                    throw new InvalidArgumentException(
                        __('Invalid hash for UUID v5', 'arrayaccess')
                    );
                }
                if (!Filter::isHex($hash)) {
                    if ($version === 5) {
                        throw new InvalidArgumentException(
                            __('Invalid hash for UUID v5', 'arrayaccess')
                        );
                        ...
                        throw new InvalidArgumentException(
                            __('Invalid hash for UUID v3', 'arrayaccess')
                        );
                    } else {
                        throw new InvalidArgumentException(
                            __('Invalid hash for SHA1 UUID', 'arrayaccess')
                        );
                    }
                }
            } else {
                ...
                if (!Filter::isHex($hash)) {
                    if ($version === 5) {
                        ...
                        throw new InvalidArgumentException(
                            __('Invalid hash for MD5 UUID', 'arrayaccess')
                        );
                    }
                }
            }
            ...
            if ($type === self::UUID_TYPE_TIME) {
                ...
                $timeLow = Consolidator::randomInt(0, 0xffffffff);
                $timeMid = Consolidator::randomInt(0, 0xffff);
                $timeHi  = Consolidator::randomInt(0, 0x0fff);
            }
            // clock sequence high and reserved
            $clockSeqHi  = Consolidator::randomInt(0, 0xffff) & 0x3f;
            $clockSeqLow = Consolidator::randomInt(0, 0xff) & 0xff;
            $node = $node??Consolidator::randomInt(0, 0xffffffffffff);
