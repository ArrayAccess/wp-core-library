...
(function (w) {
    ...
    const init = function () {
        ...
        if (typeof w.CodeJar === 'function' && (typeof w['HighlightJS'] === 'object' || typeof w['hljs'] === 'object')) {
            ...
            const codeEditorFunction = function (_this, props) {
                if (!HighLight) {
                    return;
                }
                const _height = _this.getBoundingClientRect().height;
                ...
                let wrapper = _this.parentNode.querySelector('.aa-editor-wrap');
                let innerWrapper = wrapper?.querySelector('.codejar-wrap.hljs');
                let editor = innerWrapper?.querySelector('.aa-editor-area');
                let headerWrapper = innerWrapper?.querySelector('.aa-editor-header');
                const className = 'aa-editor-wrap theme-' + theme + ' language-' + language + (
                    isResizable ? ' aa-editor-resizable' : ''
                );
                if (!editor) {
                    editor = createDiv('aa-editor-area codejar');
                }
                if (!headerWrapper) {
                    headerWrapper = createDiv(
                        'aa-editor-header',
                ...
                if (!wrapper) {
                    ...
                    if (!wrapper?.querySelector('.codejar-wrap.hljs')) {
                        wrapper.innerHTML = innerWrapper;
                    }
                    const headerLang = headerWrapper?.querySelector('.aa-header-language');
                    if (headerLang) {
                        headerLang.innerHTML = language;
                    }
                }

                const isWidget = _this.getAttribute('data-arrayaccess-widget') === 'code-editor';
                const isReadOnly = isWidget ? true : !!(_this.readOnly || _this.disabled);
                if (isReadOnly) {
                    _this.readOnly = true;
                ...
                if (typeof props === "object" && typeof props.setAttributes === "function") {
                ...
                if (!setAttributes) {
                    editor.style.height = _height + 'px';
                }
            ...
            selects('textarea[data-code-editor=codejar], textarea[data-arrayaccess-widget="code-editor"]')
            ...
            window.codeJarWindowInit = function (_textarea, props) {
                if (!_textarea
                    || !props
                    || typeof _textarea !== 'object'
                    || _textarea.tagName !== 'TEXTAREA'
                    || typeof props !== "object"
                    || typeof props.setAttributes !== "function"
                    || typeof props.attributes !== "object"
                    || !_textarea.getAttribute('data-increment-id')
                    || _textarea.getAttribute('data-increment-id').match(/[^\d.]/)
                ) {
                    return;
                }
                codeEditorFunction(_textarea, props);
            };
        }
    };
